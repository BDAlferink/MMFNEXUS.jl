...
function NEXUS_Plus(
    ...
    size(ρ) == (N, N, N) || throw(ArgumentError("size(ρ) = $(size(ρ)) does not match (N,N,N) = ($N,$N,$N)"))
    minimum(ρ) > 0       || throw(ArgumentError("ρ must be strictly positive everywhere (required for log-space filtering)"))
    L > 0 && M > 0       || throw(ArgumentError("L and M must be positive"))
    ...
    return with_logger(logger) do
        ...
        # - Integer: number of scales starting from R0 in sqrt(2) increments.
        ...
        # s_low and s_high are limiting threshold within which we look for the root. In case a root cannot be found in this range the values could be changed.
        # fraction_target is the optimal threshold for which half the objects are significant as given in the original NEXUS paper
        ...
        clean_node, clean_fila, clean_wall, clean_void
    end
end
